...
public class CourseWriter implements AutoCloseable {
	 ...
	 * for each sheet
	 */
	private final static int START_COLUMN_1 = 1;
	/**
	 * Assuming that the tables of courses starts at cells STARTCELL1 and STARTCELL2
	 * for each sheet
	 */
	private final static int START_ROW_1 = 3;

	private final static int START_COLUMN_2 = 15;

	private final static int START_ROW_2 = 3;
	 ...
	 */
	public CourseWriter(SpreadsheetDocument spreadsheetDocument, OutputStream destination, CourseSheet courseSheet)
			throws Exception {
	...
	public void writeCoursesOfYear(boolean save) throws Exception {
		...
		// Adding new sheeth
		Table sheet = this.spreadsheetDocument.appendSheet(courseSheet.getSheetMetadata().getYearOfStud());
		...
		writeSemesterCourses(courseSheet.getCourses(1), START_COLUMN_1, START_ROW_1, save);
		writeSemesterCourses(courseSheet.getCourses(2), START_COLUMN_2, START_ROW_2, save);
	}
	...
	private void writeSemesterCourses(List<Course> coursesList, int col, int line, boolean save) throws Exception {
		...
		Table sheet = this.spreadsheetDocument.getSheetByName(coursesList.get(0).getYearOfStud());
